...
class rmpParser():
  ...
  def setup_scrape(self) -> dict:
    ...
    response = requests.request("POST", BASE_URL, headers=self.headers, data=first_payload)
  ...
  def scrape_professors(self, count:int=10) -> dict:
    ...
    """
    self.payload =  PAYLOAD_PART_1 + str(count) + PAYLOAD_PART_2 + self.cursor + PAYLOAD_PART_3
    response = requests.request("POST", BASE_URL, headers=self.headers, data=self.payload)
    if response.status_code == 200:
      return self.parse_json(response.json())
    else:
  ...
  def parse_json(self, json:dict) -> dict:
    ...
    for edge in json['data']['search']['teachers']['edges']:
      ...
      prof_profile = {'department':prof_department, 'id':prof_id, 'legacyid':prof_legacyid, 'rating':prof_rating}
    ...
    self.parsed_all = not json['data']['search']['teachers']['pageInfo']['hasNextPage']
